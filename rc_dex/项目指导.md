...
## 0x7 保存区块数据：异常区块处理与恢复

### todo : 完善另一个异常处理：区块缺失
### todo ： case <-checkTicker.C:怎么理解

## 0x8 解析sol价格

### 如何获得sol价格
实时区块 -> 最新交易 -> 里面的转账（Transfer）：拿到 SOL对(USDT/USDC)的信息 -> (USDT|USDC) / SOL = 价格

### todo : 重要，需要全部理解加上注释，结合IDL文件和区块链浏览器

> #### <font color="red">语法说明1</font>
> ```go
> // ‼️这里是只获取索引的写法
> for i := range block.Transactions { 
> // ‼️通过索引访问元素并取地址，如果直接写成for i,tx := range xxx，tx是值拷贝，取&tx得到的是副本地址（临时变量的地址），不指向原数据（不是原切片的元素地址）
...
### 疑问：
1. compiledInstructions中的data在token 2022和SPL Token中怎么定义，从哪里知道data[0]是opcode的？
2. 这段代码有什么作用？
```go
...
## 0x9 解析token价格 - 事件event解析
思路： 上一节获取到sol价格后，在交易中有xx sol 换了yy token，间接就可以获得token价格  
但是获得的token数量需要从以下地方得到正确的值：
![图片](./img/Anchor%20Self%20CPI%20Log.png)
这些数据存在下面这个地方：
![图片](./img/Program%20Logs%20data.png)
Raw Data:
![pic](./img/Program%20Logs%20Raw%20Data.png) 
https://solscan.io/tx/2Q35MNAbztUogGsLJpwF8QwiCPwdVtqU9Yz8kr2xBBAgnMeAc3PYZqVn6BRoCcpmigGt5PjwQeX3X2aVXwfGrP1V
与实际字段相反，待问老师
...
### 02 token表分析
### 03 代码
新增各种DB对象，由新增的sql生成，新增token2022idl及相关  
入口上下文对象服务，新增新增的各种db对象  
然后是block.go的processblock
todo：解析pumpfun
...
func NewBuyInstruction(
	...
	// Add the accounts to the instruction.
	{
		...
		accounts__.Append(solanago.NewAccountMeta(programAccount, false, false))
		// Account 12 "global_volume_accumulator": Writable, Non-signer, Required
		accounts__.Append(solanago.NewAccountMeta(globalVolumeAccumulatorAccount, true, false))
...
// Given a budget of spendable SOL, buy at least min_tokens_out // Account creation and fees will be deducted from the spendable SOL //  // f(sol) = tokens, where tokens >= min_tokens_out and sol > rent + fees //  // max_slippage = min_tokens_out = 1 //  // Make sure the sol budget is enough to cover creation of the following accounts (unless already created): // - creator_vault: rent.minimum_balance(SystemAccount::LEN) // - user_volume_accumulator: rent.minimum_balance(UserVolumeAccumulator::LEN)
